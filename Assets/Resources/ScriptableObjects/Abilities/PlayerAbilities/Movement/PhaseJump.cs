...
public class PhaseJump : MonoBehaviour, ICanLimitMovement
{
    ...
    IEnumerator Jump()
    {
        ...
        float mouseDist = Vector3.Distance(MouseManager.instance.GetMousePosition(), player.position);
        float jumpDistance = mouseDist < maxDistance ? mouseDist : maxDistance;
        Vector3 goal = player.position + player.forward * jumpDistance;
        ...
        if (Physics.Raycast(startPos, goal - startPos, out hit, jumpDistance, BlockingMask))
        {
