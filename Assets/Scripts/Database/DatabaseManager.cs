...
{
    public delegate void LoginAttempt(); // client should maintain an implementation that is called upon successful login
    ...
    public byte[] GetUserSalt(string username)
    {
        ...
        try
        {
            ...
            Debug.Log("Salt from DBManager " + LoginBehavior.ByteArrayToString(salt));
        }
        catch (Exception e)
        {
